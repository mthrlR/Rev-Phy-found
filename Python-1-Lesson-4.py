...
email = str(input('Ur email?'))
mail_check = '[a-z_0-9]+)@[a-z-]+\.(ru|com|org)'


def is_upper(x):
    for i in x:
        if i.isupper():
            print(x, 'Данные введены корректно')
            return
        else:
            print(x, 'Имя и Фамилия должны начинаться с заглавной буквы')
            return
upper_latter = [name, surname]
for word in upper_latter:
    is_upper(word)

print(re.match(mail_check, email))
